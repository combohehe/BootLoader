...
//这个函数管理 DMA 通道1-4的分配。这个函数和 culDmaFreeChannel(...)一起记住可用的 DMA
//通道。调用这个函数时从dmaTable表搜索一个通道。如果找到一个通道，
* Parameters:
*
...
*      are configured.
//发送length字节的数据（最多122），标志，目的地址，源地址在Tx DMA 通道传送有效
//载荷到Tx FIFO前插入，如果期望应当，设置相应的标志
* Parameters:
*
...
//这个函数使能接收128字节，包括头和尾。接收数据通过DMA传输到pReceiveData。DMA装备
//同时接收开启。接收数据将触发DMA，当所有的数据包接收并且移走，DMA产生一个中断同时
//运行以前定义的函数rxCallBack
* Parameters:
*
